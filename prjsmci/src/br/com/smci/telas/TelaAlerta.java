...
public class TelaAlerta extends javax.swing.JInternalFrame {
    ...
    private void fetchData() {
        String sql = "SELECT *, GROUP_CONCAT(T3.fone SEPARATOR ', ') AS telefones\n"
                + "FROM clientes T1\n"
                + "INNER JOIN endereco T2 ON T2.idcliente = T1.idcliente\n"
                + "INNER JOIN telefones T3 ON T3.idcliente = T1.idcliente\n"
                + "WHERE T1.idcliente = ?";
        ...
        try {
            ...
            if (rs.next()) {
                ...
                txtRua.setText(rs.getString(11));
                txtNum.setText(rs.getString(12));
                txtApto.setText(rs.getString(13));
                txtComp.setText(rs.getString(14));
                txtFoneCli.setText(rs.getString(20));
            }
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, e);
        }
